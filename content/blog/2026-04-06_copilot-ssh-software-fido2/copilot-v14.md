Jetzt verstehe ich das Problem. Das Build-Skript muss die OpenSSH-Build-Infrastruktur nutzen. Hier ist die **korrigierte Lösung**:

```bash name=build-software-sk-fixed.sh
#!/bin/bash
...
Diese korrigierte Version hat folgende Verbesserungen:

1. **✅ Eigenständige C-Datei** - Keine Abhängigkeit von `includes.h` oder OpenSSH-Headern
2. **✅ SK-API Definitionen eingebettet** - Alle notwendigen Konstanten lokal definiert
3. **✅ OpenSSH-Integration** - Nutzt `crypto_api.c` korrekt
4. **✅ Bessere Fehlerbehandlung** - Prüft auf fehlendes OpenSSH-Repo
5. **✅ Automatisches Configure** - Führt `configure` aus wenn nötig
6. **✅ OpenSSL-Erkennung** - Nutzt `pkg-config` oder Fallback

**Verwendung:**

```bash
chmod +x build-software-sk-fixed.sh
./build-software-sk-fixed.sh /path/to/openssh-portable
```
...
./build-software-sk-fixed.sh ./openssh-portable
```

Das Skript wird dann:
1. OpenSSH konfigurieren
2. Standalone C-Code generieren (ohne Header-Abhängigkeiten)
3. Mit OpenSSL kompilieren
4. `libssh-sk-software.so` erzeugen
