...
Das Problem ist, dass das Programm nach "-sk" im Key-Type sucht, aber der Key-Type "sk-ssh-ed25519@openssh.com" mit "sk-" **beginnt**. Hier ist die korrigierte Version:
                            ...
                            struct ssh_key *key)
{
    ...
    /* Check if this is a FIDO key - should start with "sk-" */
    ...
    printf("Public key blob length: %u\n", len);
                                     ...
                                     size_t *keylen)
{
    ...
    /* Skip the key type prefix (e.g., "sk-ssh-ed25519@openssh.com ") */
    ...
    printf("Base64 string length: %zu\n", b64_len);
    ...
    *keylen = base64_decode(b64_str, keydata, 4096);
    ...
    printf("Decoded key data length: %zu bytes\n", *keylen);
...
1. **Erkennung von SK-Keys korrigiert**: Ändert von `strstr(keytype, "-sk")` zu `strstr(keytype, "sk-")`, da SK-Keys mit "sk-" **beginnen**.

2. **Kommandozeilen-Argument-Unterstützung**: Das Programm kann jetzt direkt den Key-String als Argument akzeptieren, nicht nur aus einer Datei.

3. **Debug-Ausgabe hinzugefügt**: Zeigt Key-Type, Längen und andere Informationen zur Fehlerdiagnose.
...
# Mit Datei
./sk_flags_decoder ~/.ssh/id_ed25519_sk.pub

# Oder direkt mit dem Key-String
