...
Gute Frage! Lassen Sie mich die OpenSSH-Quellen durchsuchen, um die interne Struktur des Key-Handles zu verstehen:Jetzt habe ich die Antwort! Der Key-Handle ist **nicht strukturiert** - es ist einfach **Rohdaten**. Lassen Sie mich das erklären:

## Aufbau des Key-Handle

Der Key-Handle ist **kein strukturiertes Format** mit sk_flags darin. Es ist einfach ein **opaker Blob** von Rohdaten (Bytes):

- **ED25519-SK**: Der Key-Handle enthält die verschlüsselte Private-Key-Information
- **ECDSA-SK**: Der Key-Handle enthält typischerweise einen PEM-codierten Private-Key

Das sehen Sie in der `sk-api.h`:
```c
struct sk_enroll_response {
    uint8_t flags;              // ← sk_flags SEPARATE vom key_handle!
    uint8_t *public_key;
    size_t public_key_len;
    uint8_t *key_handle;        // ← Opaker Blob
    size_t key_handle_len;
};
```

## Die tatsächliche SSH Public-Key-Struktur

Laut `sshkey.c` (`sshkey_serialize_sk` und `sshkey_serialize_private_sk`), ist die Struktur:

```
[keytype_string]              (length-prefixed)
[public_key]                  (length-prefixed)
[sk_application]              (length-prefixed string)
[sk_flags]                    (1 byte!)
[key_handle]                  (length-prefixed opaque blob)
[sk_reserved]                 (length-prefixed opaque blob)
```

Also: **Die sk_flags kommen NACH der application string und VOR dem key_handle!**
...
int main(int argc, char *argv[])
{
    ...
    if (sk_flags & SSH_SK_RESIDENT_KEY) {
...
Jetzt sollte es funktionieren! Die sk_flags sind ein einzelnes Byte direkt nach der `sk_application` und **VOR** dem `key_handle`!
